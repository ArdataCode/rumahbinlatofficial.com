 ...
 **/
class tags_elementor extends Widget_Base {
  ...
  'mdp-widgeter-elementor-admin',
    ...
    public function get_name() {
        ...
        return 'mdp-widgeter-tags-elementor';
    ...
    public function get_title() {
        ...
        return esc_html__( 'Tag cloud', 'selection-lite' );
    ...
    public function get_icon() {
        ...
        return 'mdp-tags-elementor-widget-icon';
    ...
    private function tab_style() {
        ...
        /** Style -> Section Style Tag Item. */
        $this->section_style_tag_item();
    ...
    private function get_taxonomies_options() {
        ...
        foreach ( $taxonomies as $tax_key => $taxonomy ) {
            if ( $tax_key === 'elementor_library_category' ) {
                $options[$tax_key] = esc_html__( 'Elementor template library category', 'selection-lite' );
            } else {
                $options[$tax_key] = $taxonomy->label;
            }
        }
    ...
    private function section_content_general() {
        ...
        $this->add_control(
            ...
            [
                ...
                'condition' => [
                    'enable_tag_cloud_title_icon' => 'yes'
                ]
            ]
        );

        $this->add_control(
            'tag_cloud_icon_position',
            [
        ...
        $this->add_control(
            'tag_cloud_taxonomy',
            [
                'label' => esc_html__( 'Taxonomy', 'selection-lite' ),
                'type' => Controls_Manager::SELECT,
                'default' => 'post_tag',
        ...
        $this->add_control(
		    'show_tag_count',
		    [
			    'label' => esc_html__( 'Show tag count', 'selection-lite' ),
			    'type' => Controls_Manager::SWITCHER,
			    'return_value' => 'yes',
			    'default' => 'no',
		    ]
    ...
    private function generate_typography_tabs_controls( $section_id, $opts = [] ) {
        $style_opts = [
            ...
            'exclude_font_size' => array_key_exists( 'exclude_font_size', $opts ) ?
                $opts['exclude_font_size'] : false,
        ...
        if ( $style_opts['include_typography'] ) {
            ...
            if ( $style_opts['exclude_font_size'] ) { $typography_settings['exclude'] = [ 'font_size' ]; }

            $this->add_group_control( Group_Control_Typography::get_type(), $typography_settings );
        }
    ...
    private function section_style_title() {
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .mdp-widgeter-tag-cloud-title' => 'width: {{SIZE}}{{UNIT}};',
                ],
            ]
        );

        $this->add_responsive_control(
            'tag_cloud_title_icon_spacing',
            [
        ...
        $this->generate_typography_tabs_controls( 'section_style_title', [
            'html_class' => 'mdp-widgeter-tag-cloud-title',
            'typography_class' => 'mdp-widgeter-tag-cloud-title',
            'additional_color' => true,
            'additional_color_name' => 'Icon color',
            'additional_color_class' => 'mdp-widgeter-elementor-title-icon',
            'additional_color_hover_class' => 'mdp-widgeter-tag-cloud-title:hover .mdp-widgeter-elementor-title-icon',
            'additional_transition_selector' => '{{WRAPPER}} .mdp-widgeter-elementor-title-icon',
            'color_class' => 'mdp-widgeter-tag-cloud-title',
            'color_hover_class' => 'mdp-widgeter-tag-cloud-title:hover',
        ] );
    ...
    /**
     * Add widget controls: Style -> Section Style Tag Item.
     *
     * @since 1.0.0
     * @access private
     *
     * @return void
     **/
    private function section_style_tag_item() {

        $this->start_controls_section( 'section_style_tag_item', [
            'label' => esc_html__( 'Tag item', 'selection-lite' ),
            'tab'   => Controls_Manager::TAB_STYLE
        ] );
        ...
        $this->generate_typography_tabs_controls( 'section_style_tag_item', [
            'html_class' => 'tagcloud a',
            'typography_class' => 'tagcloud a',
            'exclude_font_size' => true,
            'color_class' => 'tagcloud a',
            'color_hover_class' => 'tagcloud a:hover',
        ] );
    ...
    protected function render() {
       ...
       $title_tag = esc_attr( $settings['tag_cloud_title_tag'] );
       $icon_enabled = $settings['enable_tag_cloud_title_icon'] === 'yes';

        $before_widget = sprintf(
             '<div class="mdp-widgeter-elementor-box mdp-widgeter-tag-cloud-elementor-box
                    mdp-widgeter-elementor-tag-alignment-%s">',
                    esc_attr( $settings['tag_cloud_alignment'] ) );
       ...
       'tag-cloud',
            $title_tag,
            $icon_enabled,
            $settings['tag_cloud_title_icon'],
            $settings['tag_cloud_icon_position'],
            '',
            $before_widget
        );

         the_widget( 'WP_Widget_Tag_Cloud', [
             'title' => esc_html__( $settings['tag_cloud_title'], 'selection-lite' ),
             'taxonomy' => esc_attr( $settings['tag_cloud_taxonomy'] ),
             'count' => esc_attr( $settings['show_tag_count'] ) === 'yes'
         ], $default_widget_args  );

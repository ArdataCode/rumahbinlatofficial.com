...
class search_elementor extends Widget_Base {
  ...
  'mdp-widgeter-elementor-admin',
    ...
    protected function render() {
       ...
       $title_tag = esc_attr( $settings['search_title_tag'] );
       $icon_enabled = $settings['enable_search_title_icon'] === 'yes';

        $default_widget_args = Caster::get_instance()->widgeter_get_widget_default_args(
       'search',
            $title_tag,
            $icon_enabled,
            $settings['search_title_icon'],
            $settings['search_icon_position']
        );

         the_widget( 'WP_Widget_Search', [
             'title' => esc_html__( $settings['search_title'], 'selection-lite' ),
         ], $default_widget_args  );

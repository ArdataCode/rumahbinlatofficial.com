...
final class ReporterWordPress {
	...
	public function get_title() {
        ...
        return esc_html__( 'WordPress Environment', 'selection-lite' );
	...
	public function get_fields() {
		...
		return [
			'version'               => esc_html__( 'Version', 'selection-lite' ),
			'site_url'              => esc_html__( 'Site URL', 'selection-lite' ),
			'home_url'              => esc_html__( 'Home URL', 'selection-lite' ),
			'is_multisite'          => esc_html__( 'WP Multisite', 'selection-lite' ),
			'max_upload_size'       => esc_html__( 'Max Upload Size', 'selection-lite' ),
			'memory_limit'          => esc_html__( 'Memory limit', 'selection-lite' ),
			'permalink_structure'   => esc_html__( 'Permalink Structure', 'selection-lite' ),
			'language'              => esc_html__( 'Language', 'selection-lite' ),
			'timezone'              => esc_html__( 'Timezone', 'selection-lite' ),
			'admin_email'           => esc_html__( 'Admin Email', 'selection-lite' ),
			'debug_mode'            => esc_html__( 'Debug Mode', 'selection-lite' ),
		];
	...
	public function get_memory_limit() {
		...
		if ( $memory_limit_bytes < $min_recommended_bytes ) {
			$result['recommendation'] = esc_html__( 'We recommend setting memory to at least 64M. For more information, ask your hosting provider.', 'selection-lite' );
	...
	public function get_is_multisite() {
		...
		return [
			'value' => is_multisite() ? esc_html__( 'Yes', 'selection-lite' ) : esc_html__( 'No', 'selection-lite' )
		];
	...
	public function get_permalink_structure() {
		...
		if ( ! $structure ) {
			$structure = esc_html__( 'Plain', 'selection-lite' );
		}
	...
	public function get_debug_mode() {
		...
		return [
			'value' => WP_DEBUG ? esc_html__( 'Active', 'selection-lite' ) : esc_html__( 'Inactive', 'selection-lite' )
		];
